...
> each value is associated with a unique key
  ...
  * Accessing and Modifying a Dictionary
  ...
  ```
  3. add an item with subscript syntax
  ```swift
  airports["LHR"] = "London"
  // the airports dictionary now contains 3 items
  ```
  4. change an item with subscript syntax
  ```swift
  airports["LHR"] = "London Heathrow"
  // the value for "LHR" has been changed to "London Heathrow"
  ```
  5. change an item with `updateValue(_:forKey:)`
  ```swift
  ...
  ```
  6. remove value with subscript syntax
  ```swift
  ...
  ```
  7. remove value with `removeValue`
  ```swift
